...
class WorkspacesBar extends PanelMenu.Button {
	_init() {
		...
		this._onScrollEventId = this.actor.connect('scroll-event', this._onScrollEvent.bind(this));
	}
	...
	_onScrollEvent(actor, event) {
		...
		function getDirection () {
			const Is_Horizontal = false // Was: this._isHorizontal && this._settings.get_boolean('horizontal-workspace-switching')
...
class Extension {
	...
	enable() {
		...
		if (DISPLAY_APP_GRID) {
			...
			Main.panel.addToStatusArea('babar-app-grid-button', this.app_grid, 1, 'left');
		}
		...
		if (DISPLAY_TASKS) {
			...
			Main.panel.addToStatusArea('babar-workspaces-bar', this.workspaces_bar, 0, 'left');
		}
	}
